...
export const FormPage: React.FC = () => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await addDoc(collection(db, "forms"), {
        ...
        niche:          niche.trim(),
        isEmailed:      false,
        submittedAt:    serverTimestamp(), // stored as Firestore Timestamp
      });
      // ─────────────────────────────────────────────────────────────────────
  ...
  const labelStyle: React.CSSProperties = {
    ...
    color: "#e2e2e2", marginBottom: "8px", fontFamily: "'Archivo', sans-serif",
  };

  const selectStyle: React.CSSProperties = {
    ...inputStyle, paddingRight: "36px", appearance: "none" as any, cursor: "pointer",
  };

  const statusColor =
    statusKind === "error"   ? "#f87171" :
    statusKind === "success" ? "#4ade80" : "rgba(255,255,255,0.55)";
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .submit-btn {
          ...
          transition:opacity 0.2s,transform 0.1s; font-family:'Archivo',sans-serif;
      ...
      <div style={{
        ...
        <div style={{
          ...
          paddingTop: "28px", paddingBottom: "8px", position: "relative", zIndex: 1,
        }}>
        ...
        {/* Hero */}
        <div style={{
          textAlign: "center", marginTop: "52px", marginBottom: "44px",
        ...
        {/* Form Card */}
        <div style={{
          width: "100%", maxWidth: "680px", margin: "0 16px 80px", padding: "36px",
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
            ...
            <div>
              ...
              <input className="form-input" style={inputStyle} placeholder="John Doe"
                value={name} onChange={(e) => setName(e.target.value)} required disabled={isSubmitting}/>
            </div>

            <div>
              <label style={labelStyle}>Email <span style={{ color: "#a78bfa" }}>*</span></label>
              <input className="form-input" style={inputStyle} type="email" placeholder="you@example.com"
                value={email} onChange={(e) => setEmail(e.target.value)} required disabled={isSubmitting}/>
            </div>

            <div>
              <label style={labelStyle}>Social Platform <span style={{ color: "#a78bfa" }}>*</span></label>
              <div style={{ position: "relative" }}>
                <select className="form-input form-select" style={selectStyle}
                  value={platform} onChange={(e) => setPlatform(e.target.value)} required disabled={isSubmitting}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
              <div>
                ...
                <input className="form-input" style={inputStyle} placeholder="e.g., $2,000-$5,000"
                  value={monthlyRevenue} onChange={(e) => setMonthlyRevenue(e.target.value)} disabled={isSubmitting}/>
              </div>
              <div>
                <label style={labelStyle}>Followers <span style={{ color: "#a78bfa" }}>*</span></label>
                <input className="form-input" style={inputStyle} placeholder="e.g., 25,000"
            ...
            <div>
              ...
              <input className="form-input" style={inputStyle} placeholder="e.g., Tech reviews, Fitness, Business"
                value={niche} onChange={(e) => setNiche(e.target.value)} required disabled={isSubmitting}/>
            </div>

            <div>
              <label style={labelStyle}>Goals <span style={{ color: "#a78bfa" }}>*</span></label>
              <textarea className="form-input" style={{ ...inputStyle, minHeight: "130px", resize: "vertical" }}
                placeholder="Tell us what you want to achieve with Tiviala..."
                value={goals} onChange={(e) => setGoals(e.target.value)} required disabled={isSubmitting}/>
            </div>
            ...
            <p style={{ textAlign: "center", fontSize: "12px", color: "rgba(255,255,255,0.32)", marginTop: "6px", lineHeight: "1.5", maxWidth: "520px", marginLeft: "auto", marginRight: "auto" }}>
            ...
            <p style={{ textAlign: "center", fontSize: "13px", color: "rgba(255,255,255,0.28)", marginTop: "2px" }}>
              ...
              <a href="mailto:info@tiviala.com" style={{ color: "rgba(167,139,250,0.9)", textDecoration: "none" }}>
                info@tiviala.com
              </a>{" "}
            ...
            {status && (
              <p style={{ textAlign: "center", fontSize: "13px", color: statusColor }}>{status}</p>
            )}
          </form>
        </div>
      </div>
